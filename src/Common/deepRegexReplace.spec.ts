...
describe("deep regexp replace", () => {
  ...
  it("replaces by values", () => {
    ...
    expect(redacted.verySensitive).toEqual(
      `so much ${redactedPlaceholder} value inside it is dangerous`
    );
  ...
  it("works on nested objects", () => {
    ...
    expect(redacted.nested.verySensitive).toEqual(
      `so much secret value inside it is ${redactedPlaceholder}`
    );
  });
});
