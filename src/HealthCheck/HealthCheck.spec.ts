...
describe('HealthCheck', () => {
  it('Returns UP if everything is running', async () => {
    const check = new HealthCheck(new MockSlackClient(), new MockRecruiteeClient([]));
  ...
  it('Returns DOWN if SLACK is not running', async () => {
    ...
    expect(result.slack).toEqual(SlackIntegrationCheckResult.SLACK_NOT_AVAILABLE);
  });
  ...
  it('Returns DOWN if Recruitee is not running', async () => {
    const check = new HealthCheck(
      ...
      new MockRecruiteeClient([], {
        integrationCheckResult: RecruiteeIntegrationCheckResult.BAD_COMPANY_DOMAIN,
      })
    );
    const result = await check.checkHealth();
    expect(result.status).toEqual('DOWN');
    expect(result.recruitee).toEqual(RecruiteeIntegrationCheckResult.BAD_COMPANY_DOMAIN);
  });
});
